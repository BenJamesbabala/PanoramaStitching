from numpy import abs, all, any, array, arctan2, cos, sin, exp, dot, log, logical_and, roll, sqrt, stack, trace, unravel_index, pi, deg2rad, rad2deg, where, zeros, floor, full, nan, isnan, round, float32
...
def generateDoGImages(gaussian_images):
    ...
    for gaussian_images_in_octave in gaussian_images:
        ...
        for first_image, second_image in zip(gaussian_images_in_octave, gaussian_images_in_octave[1:]):
            # subtract(second_image, first_image))  # ordinary subtraction will not work because the images are unsigned integers
...
def isPixelAnExtremum(first_subimage, second_subimage, third_subimage, threshold):
    ...
    """
    center_pixel_value = second_subimage[1, 1, 1]
...
def computeHessianAtCenterPixel(pixel_array):
    ...
    dxy = 0.25 * (pixel_array[1, 2, 2] - pixel_array[1, 2,
                                                     0] - pixel_array[1, 0, 2] + pixel_array[1, 0, 0])
    dxs = 0.25 * (pixel_array[2, 1, 2] - pixel_array[2, 1,
                                                     0] - pixel_array[0, 1, 2] + pixel_array[0, 1, 0])
    dys = 0.25 * (pixel_array[2, 2, 1] - pixel_array[2, 0,
                                                     1] - pixel_array[0, 2, 1] + pixel_array[0, 0, 1])
